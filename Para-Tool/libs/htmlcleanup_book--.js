...
try {
 ...
 /* Template Css updates */
	...
	fontfamily.forEach (function (elem,i) {
		 ...
		 var fontface=elem.split(';')[0].split('",')[0].split('font-family:"')[1].replace(regex1,"__").replace("-","_").replace('"','');
	...
	varfontfamily = varfontfamily+ "@font-face {" + newfontupdate + "}\n";
	}); */
...
for(var i=0;i<$(pspanclass).length;i++)
{
	...
	function cssupdate(cssclasses) 
	{
		...
		fs.writeFileSync(output + "\\css\\" + cssfilename ,cssupdate2);
		}
