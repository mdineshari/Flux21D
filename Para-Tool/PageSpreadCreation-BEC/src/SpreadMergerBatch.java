...
public class SpreadMergerBatch {
	/**
	 * @param args
	 */
	//except first and last page all the pages should be spreads.
	//use the opf to create a merge list.
	
	//Update needed: precheck all the requirements before running the tool. and throw an error log
	...
	static ArrayList<File> tempHtmlList = new ArrayList<File>();
	
	static File contentOpf;
	static String width;
	static String height;
	static ArrayList<File>removableFiles=new ArrayList<File>();
	static ArrayList<File>allHtmls=new ArrayList<File>();
	
	static File errorLog;
	static BufferedWriter errorOut;
	
	static{
		try {
			errorLog=new File(new File(".").getCanonicalPath()+"\\Errors.log");
			BufferedWriter errorOut = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(errorLog),"UTF-8"));
	...
	public static void precheck(File titlePath) throws Exception{
		int flag=0;
		traverse(titlePath);
		Iterator<File>itr=tempHtmlList.iterator();
		while(itr.hasNext()){
			File f=itr.next();
			if(!f.getName().equals("back_cover.html") && !f.getName().equals("cover.html")){//cover and back_cover will not be merged so, no need to validate.
				Document doc=Utils.getFileAsDocument(f,"UTF-8");
				Elements styles=doc.select("link[rel=stylesheet]");
				if(styles.size()!=2){
					System.out.println("Error: css declaration missing in the file - "+f.getName());
					flag=1;
				}
				Iterator<Element>styleItr=styles.iterator();
				while(styleItr.hasNext()){
					Element e=styleItr.next();
					String href=e.attr("href");
					String fullPath=f.getParent()+"/"+href;
					File css=new File(fullPath);
					if(!css.exists()){
						System.out.println("Error: missing css physical file for "+href+" called in html - "+f.getName());
						flag=1;
					}else if(css.exists() && css.length()==0){
						//filling empty css files, as parser won't allow empty files to be merged.
						BufferedWriter out=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(css),"UTF-8"));
						out.write("			\r\n");
						out.close();
				...
				//page container
				if(getPageContainer(doc)==null){
					System.out.println("Error: missing page container div html - "+f.getName());
					flag=1;
				}
		}
		...
		if(flag==1){
			System.out.println("Info: Fix the above issues and rerun the tool.");
			System.exit(0);
		}
	}
	...
	public static void main(String ar[]) throws Exception{
		...
		File titlePath=null;
		if(ar.length==3){
			titlePath=new File(ar[0]);
			if(!titlePath.exists() && !titlePath.isDirectory()){
				System.out.println("invalid directory..");
				System.exit(0);
			}
			
			width=ar[1].replace("px", "");
			height=ar[2].replace("px", "");
		}
		else{
		...
		//precheck
		precheck(titlePath);//quits the programs if prerequisites not satisfied.
		
		
		//File titlePath=new File("D:\\BEC-new-smil-tool\\Navigator\\Our Solar System The Sun\\finalProcess\\Our Solar System The Sun");
		getJson(titlePath);
		if(json!=null){
			String jsonStr=Utils.getFileAsString(json,"UTF-8");
			pushHtmls(titlePath);
			
			jsonStr=jsonStr.replace("spreads:{","{");
			JSONParser parser=new JSONParser();
			JSONObject obj = (JSONObject)parser.parse(jsonStr);
			//System.out.println(obj);
			ArrayList<String>mergeList=new ArrayList<String>();
				for(int k=1;k<obj.size();k++){
					JSONArray arr=(JSONArray)obj.get(""+k);
					//System.out.println(arr.size());
					if(arr.size()==2){
						mergeList.add(arr.get(0)+"-"+arr.get(1));
					}
				}
				//System.out.println(mergeList);
				Iterator<String>itr=mergeList.iterator();
				while(itr.hasNext()){
				
					String filenames[]=itr.next().split("-");
					File left=null;
					File rght=null;
					
						Iterator<File>itr2=allHtmls.iterator();
						while(itr2.hasNext()){
							File f=itr2.next();
							if(f.getName().equals(filenames[0]+".html")){
								left=f;
							}
							else if(f.getName().equals(filenames[1]+".html")){
								rght=f;
							}
						}
						
					if(left!=null && rght!=null){
						cssFileList = new ArrayList<File>();
						htmlFileList = new ArrayList<File>();
						smilFileList = new ArrayList<File>();
						feedFiles(titlePath);
						singlePageProcess(left, rght);
					}
				}
		}
	...
	public static void pushHtmls(File dir){
	...
	public static void getJson(File titlePath){
		
		File list[]=titlePath.listFiles();
		for(int z=0;z<list.length;z++){
			if(list[z].isFile() && list[z].getName().equals("spread-index.json")){
				json=list[z];
				break;
	...
	public static void singlePageProcess(File left,File rght) throws Exception {
		// TODO Auto-generated method stub
		System.out.println("left:"+left);
		System.out.println("rght:"+rght);
...
//			height = propMap.get("pageheightpx");
	...
	//json recreation
	public static void createMergedJson() throws Exception{
		Document doc=Utils.getFileAsDocument(contentOpf, "UTF-8");
		Elements elist=doc.select("spine itemref");
		Iterator<Element>tempItr=elist.iterator();
		//replacing page_ to page in spine input for further use.
		while(tempItr.hasNext()){
			Element ee=tempItr.next();
			String idr=ee.attr("idref");
			ee.attr("idref",idr.replace("page_", "page"));
		}

		Iterator<Element>itr=elist.iterator();
		LinkedList<Element>spineList=new LinkedList<Element>();
		
		String jsonS="{";
		int count=0;
		while(itr.hasNext()){
			Element e=itr.next();
			String str=e.attr("idref");
			jsonS=jsonS+"\""+count+"\":[\""+str+"\"],";
			count++;
		}
		
		String finalJson=jsonS;
			   finalJson=finalJson.substring(1,finalJson.length()-1);
			   finalJson="spreads:{"+finalJson+"}";
			   
		File spreadJson=new File(contentOpf.getParent()+"/spread_index/spread-index.json");	   
		
		BufferedWriter out=new BufferedWriter(new OutputStreamWriter(new FileOutputStream(spreadJson),"UTF-8"));
		out.write(finalJson);
		out.close();
	}
	...
	public static void cleanUp(){
		...
		while(itr.hasNext()){
			File f=itr.next();
			if(f!=null){
				if(f.exists() && f.getName().endsWith(".html")){
	...
	public static String processCSS(File css1, File css2) throws Exception {
		...
		return cssName;
			...
			System.out.println("Error: Css file not available..");
			if(!css1.exists()){
				e.printStackTrace();
				errorOut.write("Css File Not Found: "+css1+"\r\n");
			}
			if(!css2.exists()){
				errorOut.write("Css File Not Found: "+css2+"\r\n");
			}
		}
		return "";
	}
	...
	static void traverse(File dir)throws Exception{
		
		File list[]=dir.listFiles();
		for(int i=0;i<list.length;i++){
			if(list[i].isFile() && list[i].getName().endsWith(".html")){
				tempHtmlList.add(list[i]);
			}else if(list[i].isDirectory()){
				traverse(list[i]);
			}
		}
